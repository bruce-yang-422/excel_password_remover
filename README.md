...
- 檔案：`any_name.zip`（密碼：17279274）
- 自動匹配：`password: "17279274"` 的設定
- 解壓縮到：`output/any_name_東森購物_extracted/`
...
3. **檔案格式**：支援 Excel 檔案格式（.xlsx, .xls）和壓縮檔案格式（.zip, .rar）
4. **檔名規則**：Excel 檔名必須包含對應的 `account` 字串，壓縮檔案可使用任何檔名
5. **未加密檔案**：程式會自動處理未加密的檔案，直接複製到輸出資料夾
6. **RAR 檔案支援**：Windows 系統需要安裝 WinRAR 或 7-Zip 來支援 RAR 檔案處理
7. **密碼匹配順序**：程式會按照 `compressed_files` 的順序嘗試密碼
...
本專案由宜加寵物用品 Bruce 開發，版本 1.3 (2025-08-01)